...
BloomFilterBlurHor::BloomFilterBlurHor(ID3D11Device* device, HWND hwnd, int w, int h) : BaseShader(device, hwnd)
{
	sWidth = w;
	sHeight = h;
	initShader(L"bloom_filter_blur_hor_cs.cso", NULL);
}

BloomFilterBlurHor::~BloomFilterBlurHor()
{

}

void BloomFilterBlurHor::initShader(const wchar_t* cfile, const wchar_t* blank)
{
...
void BloomFilterBlurHor::createOutputUAV()
{
...
void BloomFilterBlurHor::setShaderParameters(ID3D11DeviceContext* dc, ID3D11ShaderResourceView* texture1, int radius, float threshold, float* kernel)
{
...
void BloomFilterBlurHor::unbind(ID3D11DeviceContext* dc)
{
