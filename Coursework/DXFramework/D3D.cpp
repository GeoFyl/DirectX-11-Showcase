...
void D3D::createDefaultRasterState()
{
	...
	device->CreateRasterizerState(&rasterDesc, &rasterStateCullNone);
...
void D3D::setBackfaceCulling(bool b)
{
	if (!wireframeState) {
		cullBackFaceState = b;
		if (cullBackFaceState)
		{
			deviceContext->RSSetState(rasterState);
		}
		else
		{
			deviceContext->RSSetState(rasterStateCullNone);
		}
	}

}

bool D3D::getBackfaceCulling()
{
	return cullBackFaceState;
}
